...
├── review_generator_v4.py  # 综述生成模块（Prompt v4）
├── prompt_template.py      # Prompt模板
├── prompt_template_v2.py   # Prompt模板（v2版本）
├── prompt_template_v3.py   # Prompt模板（v3版本，章节+引用增强）
├── prompt_template_v4.py   # Prompt模板（v4版本，基于v2融合v3规范）
├── api_service_v2.py       # API服务（v2版本）
├── api_service_v3.py       # API服务（v3版本）
├── api_service_v4.py       # API服务（v4版本）
├── test_api.py             # API测试脚本
...
## v3方案：章节+引用增强的纯Prompt方案

### 概述

v3方案在v2方案的基础上，增强了章节结构规范、引用格式和参考文献格式要求，确保生成的文献综述更符合学术规范。

### 核心特点

1. **章节编号规范**：所有章节标题必须带数字编号（如`## 2 模型与架构演进`），但"参考文献"章节不带编号
2. **叙述型引用**：正文中必须使用"作者+年份+编号"的叙述型引用格式（如"Radford等人 [3] 提出了..."）
3. **IEEE格式参考文献**：参考文献必须按照IEEE格式规范编写
4. **关键词格式**：摘要后必须包含技术性关键词，英文综述第一个关键词首字母大写
5. **标题格式**：综述标题必须使用一级Markdown标题（`# Title`）
...
```
ICAIS2025-LiteratureReview/
├── api_service_v3.py          # FastAPI主应用（v3版本）
├── review_generator_v3.py     # 综述生成器（v3版本）
├── prompt_template_v3.py      # Prompt模板（v3版本）
├── ...                        # 其他文件（复用原有文件）
```
...
| 特性 | v2方案 | v3方案 |
|------|--------|--------|
| 章节编号 | 无强制要求 | 必须带数字编号（除参考文献） |
| 引用格式 | 简单引用 | 叙述型引用（作者+年份+编号） |
| 参考文献格式 | 基础格式 | IEEE格式规范 |
| 关键词格式 | 基础格式 | 技术性术语，英文首字母大写 |
| 标题格式 | 二级标题 | 一级标题 |
| 字数要求 | 6000-8000字 | 5000-6000字 |
...
```bash
python api_service_v3.py
```

#### Docker运行

当前Dockerfile默认部署v3方案，如需切换版本，请修改Dockerfile中的相关配置。

## v4方案：基于v2结构融合v3规范

### 概述

v4方案是v2和v3方案的融合版本，**以v2版本的详细结构和内容要求为基础**，**融合v3版本的格式规范**（章节编号、引用格式、参考文献格式、关键词格式等），生成既详实又规范的文献综述。
...
3. **最佳平衡**：既保证了内容的详实性和深度，又确保了格式的学术规范性
...
```
ICAIS2025-LiteratureReview/
├── api_service_v4.py          # FastAPI主应用（v4版本）
├── review_generator_v4.py     # 综述生成器（v4版本）
├── prompt_template_v4.py      # Prompt模板（v4版本）
├── ...                        # 其他文件（复用原有文件）
```
...
**注意**：v4方案使用相同的端点，但需要运行`api_service_v4.py`。
...
2. **阶段2：生成文献综述**
   - 基于知识规划生成完整的结构化综述
   - 遵循v3的格式规范（章节编号、引用格式等）
   - 保持v2的内容详实性（详细的技术分析、对比表格等）
...
```bash
python api_service_v4.py
```

#### Docker运行

当前Dockerfile默认部署v4方案。如需切换版本，请修改Dockerfile中的相关配置。
...
| 内容详实性 | ⭐⭐⭐⭐⭐ | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| 格式规范性 | ⭐⭐⭐ | ⭐⭐⭐⭐⭐ | ⭐⭐⭐⭐⭐ |
| 字数要求 | 6000-8000字 | 5000-6000字 | 6000-8000字 |
| 表格数量 | 4-5个 | 2-3个 | 4-5个 |
| 章节编号 | 无强制要求 | 必须带编号 | 必须带编号 |
| 引用格式 | 简单引用 | 叙述型引用 | 叙述型引用 |
| 参考文献 | 基础格式 | IEEE格式 | IEEE格式 |
| 推荐场景 | 需要详细内容 | 需要规范格式 | 需要详细且规范 |

### 推荐使用场景

- **v2方案**：需要非常详细和深入的内容分析，对格式要求不严格
- **v3方案**：需要规范的学术格式，内容可以相对简洁
- **v4方案**：**推荐使用**，既需要详细内容又需要规范格式（最佳平衡）
